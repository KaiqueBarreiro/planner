...
export function Activities({ tripDetails }: Props) {
    ...
    async function hanldeCreateTripActivity() {
        try {

            if ( !activityDate || !activityDate || !activityTitle ) {
                return Alert.alert("Cadastrat atividade", "Preencha todos os campos!");
            }
        ...
        try {
            ...
            const activitiesToSectionList = activities.map((dayActivity) => ({
                title: {
                    ...
                    dayName: dayjs(dayActivity.date).format("ddds").replace("-feira", ""),
                },
    ...
    useEffect(() => {getTripActivities()}, [])

    return (<View className="flex-1">
                <View className="w-full flex-row mt-5 mb-6 items-center">
                    <Text className="text-zinc-50 text-2x1 font-semibold flex-1">
                        Atividades
                    </Text>
                ...
                <Modal title="Cadastrar atividade" subtitle="Todos os convidados podem visualizar as atividades." visible={showModal === MODAL.NEW_ACTIVITY} onClose={() => setShowModal(MODAL.NONE)}>
                    ...
                    <Button onPress={() => hanldeCreateTripActivity} isLoading={isCreatingActivity}>
